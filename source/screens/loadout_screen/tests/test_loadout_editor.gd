...
func test_standalone_mount_self_binds_a_default_session() -> void:
	# The main-menu loadout route (and F6 dev-run) mount this scene on its own. Without a self-bind the
	# board renders empty — the Quick Match regression. The scene must bind a fresh default game in _ready.
	var scene := _scene()
...
func test_tapping_a_module_focuses_it() -> void:
	...
	var scene := _scene()
	var module: ModuleState = GameSession.game_state.starship.loadout.modules[0]
