...
func _build() -> void:
	...
	for entry: Dictionary in _data_catalogs():
		...
		data.add_row(DebugRow.nav(catalog.catalog_name, "", func() -> void: _push(CatalogView.create(catalog, factory))))
	add_child(data)
...
func _data_catalogs() -> Array[Dictionary]:
	var entries: Array[Dictionary] = [
		{"catalog": Catalogs.modules, "factory": func(entry: Resource) -> DebugView: return ModuleDetailView.create(entry as ModuleBlueprint)},
		{"catalog": Catalogs.module_grids, "factory": func(entry: Resource) -> DebugView: return ModuleGridDetailView.create(entry as ModuleGridBlueprint)},
		{"catalog": Catalogs.starships, "factory": func(entry: Resource) -> DebugView: return StarshipDetailView.create(entry as StarshipBlueprint)},
		{"catalog": Catalogs.rules, "factory": func(entry: Resource) -> DebugView: return MatchRulesView.create(entry as Ruleset)},
		{"catalog": Catalogs.match_configs, "factory": func(entry: Resource) -> DebugView: return MatchConfigDetailView.create(entry as MatchConfig)},
