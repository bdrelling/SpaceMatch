...
# start_match records the given config as the board config the next board reads, so its geometry and ruleset
# drive the mounted match, then opens the encounter.
func test_start_match_uses_the_given_config() -> void:
	var config := MatchConfig.new()
	config.ruleset = RuleCatalog.default_ruleset()
	config.board_width = 10
	var encounter: Encounter = auto_free(GameCoordinator.start_match(config))
	assert_object(encounter.state).is_not_null()
	assert_object(DebugConfig.match_config).is_same(config)
	assert_object(DebugConfig.match_ruleset).is_same(config.ruleset)
	DebugConfig.match_config = null  # clear the override so later tests read the catalog's active config
