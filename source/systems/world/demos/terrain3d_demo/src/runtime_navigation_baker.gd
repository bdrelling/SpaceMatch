...
func _exit_tree() -> void:
	# A bake may be in flight, or completed but not yet collected (_bake_finished is
	# call_deferred). Either way the pool still holds the task's Callable until it is
	# waited on, so retire any non-negative id before the engine tears down.
	if _bake_task_id != -1:
...
func _process(p_delta: float) -> void:
	...
	if player is CharacterBody3D:
		...
		track_pos += body.velocity * bake_cooldown
