...
const _COLLISION_SET: TileCollisionSet = preload("res://systems/match/tile_collision_set.tres")
...
func _attach_body(tile: MatchTile, kind: int, object: GridObjectState, position: Vector2) -> void:
	...
	# Tiles pile on each other and rest against the board walls.
	body.collision_layer = CollisionLayer.TILES
	body.collision_mask = CollisionLayer.TILES | CollisionLayer.WALLS
	add_child(body)
...
func _add_wall(center: Vector2, size: Vector2) -> void:
	...
	# Walls only need to catch the falling tiles.
	wall.collision_layer = CollisionLayer.WALLS
	wall.collision_mask = CollisionLayer.TILES
