...
## TileSpawnRule + nested/resolved [Ruleset]: a spawn rule contributes its tile's kind at a weight, the board pool
## is every spawn rule composed (the match's plus a starship's, folded together), and two rules for the same tile
## merge by their combine_mode — REPLACE retunes, STACK adds. This is the generic spawn path: nothing keys off a
## resource, and the kind set isn't a fixed count.

# A TileSpawnRule for tile [param kind] at [param weight], merging per [param mode] on a same-tile clash.
func _spawn(kind: int, weight: int, mode: Rule.CombineMode = Rule.CombineMode.REPLACE) -> TileSpawnRule:
	var tile := Tile.new()
	tile.kind = kind
	var rule := TileSpawnRule.new()
	rule.tile = tile
	rule.weight = weight
	rule.combine_mode = mode
	return rule

func test_spawn_contribution_maps_tile_kind_to_weight() -> void:
	var table: Dictionary = _spawn(3, 15).spawn_contribution()
	assert_int(table[3]).is_equal(15)

func test_non_board_tile_contributes_nothing() -> void:
	# A tile with no kind (-1) isn't a board tile, so it adds nothing to the pool.
...
func test_stack_sums_same_tile_weight() -> void:
...
func test_replace_overrides_same_tile_weight() -> void:
...
func test_flatten_folds_child_rulesets_before_own_rules() -> void:
	...
	assert_int((flat[0] as TileSpawnRule).tile.kind).is_equal(0)
	assert_int((flat[1] as TileSpawnRule).tile.kind).is_equal(1)
