...
extends GridTile
## A component tile for the match-3 board: one kind per authored [Tile] (combat, propulsion, science, shields,
## scrap, warp, damage, plus any added in-editor), chosen by [member kind] and drawn centred on the node origin
## in unit cell-space so a pop scales about the centre. Its sprite, colour and label come from the [Tile] bound to
## its kind (see [method _tile_for]) — the tiles own their representation, and the set of kinds comes from the tile
## catalog ([method TileCatalog.tile_kinds]) rather than a fixed count. The damage tile has no sprite yet, so it
## falls back to a hand-drawn starburst. A non-zero [member tint] modulates the art.
...
#region Properties
# Tile kind -> its Tile, built once from the tile catalog. The tiles own the art/colour/label; this just indexes
# them by kind for the board.
...
#region Methods
# The Tile bound to [param kind], or null when none maps to it.
static func _tile_for(kind: int) -> Tile:
	if _by_kind.is_empty():
		for tile: Tile in Catalogs.tiles.tiles:
			if tile != null:
				_by_kind[tile.kind] = tile
...
static func bake_collision_outlines(alpha_threshold: float, epsilon: float) -> Array[PackedVector2Array]:
	...
	for kind: int in Catalogs.tiles.tile_kinds():
		...
		if tile != null and tile.texture != null:
			outlines.append(_trace_outline(tile.texture, alpha_threshold, epsilon))
	return outlines
