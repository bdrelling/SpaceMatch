...
## Board gestures must dispatch through the minigame's own input routing — any controls
## above the board (the screen frame, its bars) otherwise swallow pointer events before
## _unhandled_input. A fixed-size SubViewport pins the coordinate space so pushed touches
