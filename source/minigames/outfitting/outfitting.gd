...
extends Minigame
## Outfitting minigame: pack healthy modules drawn from the outpost's salvage yard into a ship's
## irregular module bay. The ship grid (the board, in a [BoardCanvas]) is a [ShipModuleGrid] — a
## rectangular bounds with a carved hull silhouette of at least 24 usable cells. The bottom strip
## ([ModuleStrip]) shows the yard's healthy modules as their footprints; packing IS the interface here,
## so the shell pins it open.
##
## Tap a module in the strip to pick it up, then drag a finger across the bay to position its footprint
## preview and lift to slot it (drawing one from the yard). While a module is held, a second-finger tap
## rotates it (desktop uses [code]rotate_item[/code]). Tap a slotted module with nothing held to return
## it to the yard.

## Ship bay layout, authored as a [ShipModuleGridBlueprint]: a 6x6 bounds carved into a hull
## silhouette (28 usable cells), irregular but always solvable.
...
# Starter modules seeded into an empty salvage yard so there's something to pack.
...
# The salvage yard's healthy modules ARE the detail; the shell pins this strip open below the bay.
...
## Binds the minigame to the running game's outpost: it packs modules drawn from the outpost's
## salvage yard (the shared economy), seeding a few when the yard is empty. Called by [MinigameScreen]
## when the arcade mounts this page. The player inventory is unused here.
func bind_session(session: GameSession, _inventory: Inventory) -> void:
	var yard_state := _yard_state(session)
	if yard_state == null:
		return
	if _salvage_yard == null:
		_salvage_yard = Inventory.new()
		add_child(_salvage_yard)
		_salvage_yard.changed.connect(_on_yard_changed)
...
func _build_ship() -> ShipModuleGrid:
	var blueprint: ShipModuleGridBlueprint = _SHIP_GRID_BLUEPRINT
	return blueprint.create()

func _seed_starter_modules() -> void:
	if _salvage_yard == null:
		return
	for stack: ItemStack in _salvage_yard.get_stacks():
	...
	for path: String in _STARTER_MODULES:
		...
		_salvage_yard.add_variant(blueprint, [], 1)

#region Module strip

func _on_yard_changed(_inventory: Inventory) -> void:
	_rebuild_modules()
	_update_status()

# Feeds the strip the yard's fully healthy modules and re-lights the held one.
...
# dragged across the bay with finger 0 and slotted on release; with nothing held, a press pulls the
# slotted module back to the yard. Second-finger rotate is handled in [method _input].
...
func _try_place(cell: Vector2i) -> void:
	...
	_salvage_yard.remove_from_stack(_held_stack, 1)
	_cancel_hold()
	_update_status()
...
func _pull_module(cell: Vector2i) -> void:
	...
	if _salvage_yard != null and blueprint != null:
		_salvage_yard.add_variant(blueprint, [], 1)
	_update_status()
...
func _update_status() -> void:
	if _held_stack != null:
		status_text = "Placing %s — drag on the bay, two-finger tap to rotate, lift to slot." % _held_stack.item_blueprint.name
	else:
		status_text = "Bay: %d / %d cells filled. Tap a module below to pick it up." % [
