...
func apply(context: RuleContext) -> void:
	...
	if starship == null:
		return
	for kind: int in match_context.counts:
		...
		if match_context.actor_stats != null and stat != -1:
			reward += maxi(0, match_context.actor_stats.value(stat))
		# Banks into the mover's tally, clamped to its capacity for this kind (unlimited by default).
		starship.add_resource(kind, reward)
