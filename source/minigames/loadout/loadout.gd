...
func _ready() -> void:
	# Stand alone unless mounted as a game page: when this scene runs on its own — the dev preview
	# (F6) or the main-menu loadout route, both reached via [SceneLoader] which never sets the tree's
	# current_scene — no [MinigameScreen] binds a session, so bind a fresh default game here so the
