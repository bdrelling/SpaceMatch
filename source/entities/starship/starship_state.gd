...
extends Resource
## A starship: its name and the module grid loaded into it.
...
## A fight copy of this starship for an encounter: its own [member stats] block and [member health], so combat and
...
## The starship's max hull — its base health stat plus every hull module's, the cap [member health] starts at and
## the bar's top. Derived from the starship (stats + modules), so editing the hull stat or slotting a hull module
