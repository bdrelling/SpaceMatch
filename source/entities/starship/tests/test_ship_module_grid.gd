...
func test_placed_at_returns_footprint_for_any_covered_cell() -> void:
	...
	var placed := grid.placed_at(Vector2i(1, 0))
	assert_object(placed).is_not_null()
	assert_object(placed.module).is_same(module)
	assert_int(placed.cells.size()).is_equal(2)
	assert_object(grid.placed_at(Vector2i(2, 2))).is_null()
...
func test_disabled_cell_drops_its_module_from_the_profile() -> void:
	...
	assert_bool(grid.placed_modules()[0].enabled).is_true()
	...
	assert_bool(grid.placed_modules(disabled)[0].enabled).is_false()
...
func test_disabling_any_cell_deactivates_a_multi_cell_module() -> void:
	...
	assert_bool(grid.placed_modules([Vector2i(1, 0)])[0].enabled).is_false()
