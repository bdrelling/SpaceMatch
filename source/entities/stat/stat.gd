class_name Stat
## The starship stats a module contributes to. A module's contribution is authored as a [StarshipStats];
## a starship's profile is the sum of its slotted modules' blocks.

## DAMAGE backs the attack tile — it's the starship's combat stat. Scrap and Warp are not starship stats: the scrap
## tile banks to the wallet (currency) and the warp tile charges the encounter meter — neither a starship profile.
...
## The starship stat a match tile [param kind] is bound to, or -1 when the tile carries no stat (scrap, warp).
