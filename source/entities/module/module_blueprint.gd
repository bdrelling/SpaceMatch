...
## starship's ability set while the module is enabled (see [method StarshipLoadout.abilities]).
@export var abilities: Array[Ability] = []
## Phase rules this module grants its starship — board behaviour that rides on having the module installed.
## Aggregated into the starship's rules while the module is enabled (see [method StarshipLoadout.rules]).
