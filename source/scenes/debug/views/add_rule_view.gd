...
func _build() -> void:
	var note := Label.new()
	note.text = "Rule catalog — coming with the catalogs work.\n\nA rule is a configurable effect (e.g. \"Combat tiles deal damage to all\"). Picking one here adds it: single-slot categories (like scoring) replace the current rule; stackable categories append. Starship rules will take precedence over match rules."
