...
## on the right — no title, since each stage now carries its own header. The bar itself is transparent
...
# The scrap counter's count label — the chip (tile glyph + count) is authored in the scene; this
# script only repaints the count. The glyph kind matches the board's scrap tile via the scene.
...
func set_actions(actions: Array[MinigameAction]) -> void:
	...
	for action: MinigameAction in actions:
		...
		button.theme_type_variation = &"BarButton"
