...
## The Arcade game shell: the idle clock drips into the session-bound inventory, two screens bound
...
func test_arcade_boots_a_session_and_is_2d_only() -> void:
	var arcade := Arcade.create()
	add_child(arcade)
	await await_idle_frame()
	assert_object(arcade.session).is_not_null()
	assert_int(arcade.session.state.players.size()).is_equal(1)
	assert_bool(_has_node3d(arcade)).override_failure_message(
		"Arcade must be 2D-only, but a Node3D was found in its tree.").is_false()
	arcade.queue_free()
...
func test_pager_has_every_stage_and_settings() -> void:
	...
	for screen: ArcadeScreen in arcade._pager.screens:
		titles.append(screen.title)
	# Order is the scene's business; the shell just has to surface the outpost, every stage, and Settings.
	assert_array(titles).contains_exactly_in_any_order(
		["Outpost", "Salvaging", "Recycling", "Fabricating", "Outfitting", "Settings"])
	arcade.queue_free()
...
func test_selecting_a_tab_pages_to_it() -> void:
	...
	arcade._tab_bar.tab_selected.emit(2)
	assert_bool(arcade._pager.screens[2].visible).is_true()
	assert_bool(arcade._pager.screens[0].visible).is_false()
	arcade.queue_free()
...
func test_settings_cog_pages_to_settings() -> void:
	...
	arcade._top_bar.settings_pressed.emit()
	var settings_index := -1
	for index: int in arcade._pager.screens.size():
		if arcade._pager.screens[index].title == "Settings":
			settings_index = index
	assert_int(settings_index).is_greater_equal(0)
	assert_bool(arcade._pager.screens[settings_index].visible).is_true()
	arcade.queue_free()

func test_trackpad_pan_pages_between_screens() -> void:
	var pager := ArcadePager.new()
	add_child(pager)
...
func test_tabs_are_not_keyboard_focusable() -> void:
	...
	add_child(arcade)
	await await_idle_frame()
	var tabs := arcade._tab_bar.tab_buttons()
	for button: Button in tabs:
		assert_int(button.focus_mode).is_equal(Control.FOCUS_NONE)
	# One tab for the outpost plus each playable stage; Settings is the cog, not a tab.
...
func test_minigame_screens_do_not_block_board_input() -> void:
	...
	assert_int(minigame_screens).is_equal(5)
	arcade.queue_free()
...
func test_restart_replaces_the_session() -> void:
	...
	add_child(arcade)
	await await_idle_frame()
	var first := arcade.session
	arcade.restart()
	assert_object(arcade.session).is_not_same(first)
	arcade.queue_free()
