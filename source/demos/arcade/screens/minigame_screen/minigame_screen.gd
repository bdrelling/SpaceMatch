...
## An arcade page that hosts a self-contained minigame. The minigame is authored as the page's child
...
## method; this screen forwards the session and shared inventory to it on mount.

var _minigame: Node

func _ready() -> void:
	# The minigame is the page's authored child (see arcade.tscn) — nothing to instantiate here.
...
func bind(session: GameSession, inventory: Inventory) -> void:
	if _minigame != null and _minigame.has_method("bind_session"):
		...
		_minigame.call("bind_session", session, inventory)
